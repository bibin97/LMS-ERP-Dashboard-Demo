...
const StudentLayout = () => {
    ...
    const navItems = [
        ...
        { path: '/student/schedule', icon: <Calendar size={18} />, label: 'Schedule' },
        { path: '/student/messages', icon: <MessageSquare size={18} />, label: 'Messages' },
        { path: '/student/achievements', icon: <Trophy size={18} />, label: 'Achievements' },
        { path: '/student/profile', icon: <User size={18} />, label: 'Profile' },
    ];

    return (
        <div className="flex min-h-screen relative overflow-hidden bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
            {/* Mobile Overlay */}
            {isSidebarOpen && (
                <div 
